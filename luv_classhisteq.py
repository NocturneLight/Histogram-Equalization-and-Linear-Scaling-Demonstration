...
def _ClassHistogramEqualization(image, width1, height1, width2, height2):
    ...
    # Manually calculate the histogram by getting the number 
    # of occurrences of each value in the luminosity channel.
    ...
    #histogram = cv2.calcHist([imgCopy], [0], mask, [256], [0, 256])
    ...
    for i in range(height1, height2 + 1):
        for j in range(width1, width2 + 1):
            ...
            # Replace the current luminance value in the image
            # with the equalized value.
            imgCopy[i, j][0] = equalHisto[l]

    # Convert the image back to BGR from the LUV color space.
