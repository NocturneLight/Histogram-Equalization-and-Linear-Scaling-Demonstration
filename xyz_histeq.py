...
def _OpenCVHistogramEqualization(image, width1, height1, width2, height2):
    ...
    # convert it to the XYZ color space.
    ...
    for i in range(height1, height2 + 1):
        for j in range(width1, width2 + 1):
            # Store the x, y, and z channels of the pixel inside variables.
            x, y, z = imgCopy[i, j]

            # Next, we compute a grayscale value for the pixel.
            #grayValue = round(0.3 * x + 0.6 * y + 0.1 * z + 0.5)

            # Store the pixels inside the user-defined area in the image slice.
            imgSlice[i, j] = y #[x, y, z]
    ...
    imgCopy[height1:height2, width1:width2, 1] = imgSlice[height1:height2, width1:width2]

    # Convert the image back to BGR from XYZ.
